locals {
  dns_name_preffix = "spark-${join("", var.cluster_environments)}"
  dns_name = "${local.dns_name_preffix}.${var.cluster_instance_name}.${var.cluster_dns_name}"
  secret_name = "${local.dns_name}-tls"
}

# For more info view:
# https://artifacthub.io/packages/helm/bitnami/spark
resource "helm_release" "spark" {
  name             = "data-engine"
  namespace        = var.namespace

  repository = "https://charts.bitnami.com/bitnami"
  chart      = "spark"

  values = [
    "${file("${path.module}/values.yaml")}"             
  ]

  set {
     name  = "ingress.hostname"
     value = local.dns_name
  }

  set {
    name  = "master.configOptions"
    value = "-Dspark.ui.reverseProxy=true -Dspark.ui.reverseProxyUrl=https://${local.dns_name}"
  }

  set {
    name  = "worker.configOptions"
    value = "-Dspark.ui.reverseProxy=true -Dspark.ui.reverseProxyUrl=https://${local.dns_name}"
  }
  
  # set {
  #   name  = "worker.configOptions"
  #   value = jsonencode({
  #     "-Dspark.ui.reverseProxy" = "true"
  #     "-Dspark.ui.reverseProxyUrl" = "https://${local.dns_name}"
  #   })
  # }


}

